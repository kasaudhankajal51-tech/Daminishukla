...
export function CreatorClientPage({ bannerUrl }: { bannerUrl: string }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-inter selection:bg-sky-200 selection:text-sky-900 pb-20">
      ...
      {/* Social Feed (Phase 1: YouTube Placeholder) */}
      <section className="max-w-6xl mx-auto px-4 py-24">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold font-outfit mb-4 text-slate-900">Latest Content</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-sky-400 to-cyan-300 mx-auto rounded-full" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[1, 2, 3].map((item) => (
            <div key={item} className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-md hover:shadow-xl transition-all group">
              <div className="aspect-video bg-slate-200 relative overflow-hidden">
                <img src="/creator-bg.png" alt="Video thumbnail" className="w-full h-full object-cover opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" />
                <div className="absolute bottom-3 right-3 bg-slate-900/80 text-white px-2.5 py-1 text-xs rounded-md font-medium backdrop-blur-sm">10:42</div>
              </div>
              <div className="p-6">
                <h3 className="font-bold text-lg mb-2 text-slate-800 line-clamp-2 group-hover:text-sky-600 transition-colors">The Ultimate Lifestyle Guide for 2026 | Damini Shukla</h3>
